...
parser.add_argument('--iters', type=int, default=55*1000,
...
def minialexnet(data, labels=None, train=False, param=learned_param,
    ...
    n.conv6, n.relu6 = conv_relu(n.pool5, 3, 256, pad=1, group=2, **conv_kwargs)
    ...
    n.fc9, n.relu9 = fc_relu(n.drop8, 1024, param=param)
    n.drop9 = L.Dropout(n.relu9, in_place=True)
    preds = n.fc10 = L.InnerProduct(n.drop9, num_output=num_classes, param=param)
    if not train:
        # Compute the per-label probabilities at test/inference time.
        preds = n.probs = L.Softmax(n.fc10)
    if with_labels:
        n.label = labels
...
def train_net(with_val_net=False):
    ...
    solver.restore('/home/ubuntu/CS280MiniPlaces/snapshot_cfla/place_net_iter_15000.solverstate')
