...
parser.add_argument('--snapshot_dir', default='./snapshot_nda',
    help='Path to directory where snapshots are saved')
parser.add_argument('--snapshot_prefix', default='place_net_nda',
...
def miniplaces_net(source, train=False, with_labels=True):
    ...
    transform_param = dict(mean_value=mean)
    batch_size = args.batch if train else 100
    places_data, places_labels = L.ImageData(transform_param=transform_param, source=source, root_folder=args.image_root, shuffle=train,
...
def eval_net(split, K=5):
    ...
    filename = 'top_%d_predictions_nda.%s.csv' % (K, split)
